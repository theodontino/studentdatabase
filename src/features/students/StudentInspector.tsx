...
export function StudentInspector({ workspace }: { workspace: Workspace }) {
  ...
  if (!student) return <aside className="student-inspector student-inspector--empty"><EmptyState title="悬停一名学生" description="在左侧名单停留 120ms 即可查看本学期摘要；点击直接打开完整档案。" /></aside>;
  ...
  >
    ...
    <section className="student-inspector__score">
      <span>本学期综合分</span>
      <strong>
        <span className="student-inspector__score-current">{summary?.score100 ?? "—"}</span>
        {summary?.score100 != null && <small className="student-inspector__score-maximum">/100</small>}
      </strong>
