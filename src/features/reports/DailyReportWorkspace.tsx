...
export default function DailyReportWorkspace() {
  const [semesterId, setSemesterId] = useState(""); const [className, setClassName] = useState(""); const [sessionCode, setSessionCode] = useState("");
  ...
  function restore(state: DailyHistoryState) { setSemesterId(state.semesterId); setClassName(state.className); setSessionCode(state.sessionCode); setReport(state.report); setError(""); }
