...
interface BatchFeedbackHistoryState {
  kind: "batch";
  semesterId: string;
  sessionCode: string;
  className: string;
...
interface SingleFeedbackHistoryState { kind: "single"; semesterId: string; className: string; studentId: string; sessionCode: string; days: number; feedback: string; }
type FeedbackHistoryState = BatchFeedbackHistoryState | SingleFeedbackHistoryState;
...
interface ParsedResult { students: ParsedStudent[]; alert_suggestion?: string; }
...
export default function FeedbackWorkspace() {
  ...
  useEffect(() => {
    fetch("/api/students").then((response) => response.json()).then((items: Array<{ id: string; name: string; class: string }>) => setStudents(items)).catch(() => setStudents([]));
  }, []);
  ...
  useEffect(() => {
    ...
    async function loadFeedbackContext() {
      ...
      try {
        ...
        if (!cancelled) {
          setContextStudents([]);
          setContextError(errorMessage(e, "读取反馈上下文失败"));
        }
      } finally {
  ...
  async function createSession() {
    ...
    try {
      ...
      setError(errorMessage(error, "新建课次失败"));
    } finally {
  ...
  async function handleAssistantRosterImport(files: FileList | null) {
    ...
    try {
      ...
      setError(errorMessage(e, "助教表解析失败"));
    } finally {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      setError(errorMessage(e, "确认写入失败"));
    } finally {
      setConfirming(false);
    }
  }
  ...
  async function generateSingleFeedback() {
    ...
    setSingleLoading(true); setError("");
    try {
      const body = sessionCode ? { studentId: singleStudentId, sessionCode } : { studentId: singleStudentId, days: singleDays };
      const response = await fetch("/api/report/feedback", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      const data: { feedback?: string; error?: string } = await response.json();
      if (!response.ok) throw new Error(data.error || "生成单人反馈失败");
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <WorkHistoryButton<FeedbackHistoryState> modules={FEEDBACK_HISTORY_MODULES} accept={isFeedbackHistoryState} onRestore={restoreHistory} />
      </div>
      ...
      <section className="rounded-lg border border-gray-200 bg-white p-5">
        <div className="mb-4"><h3 className="font-semibold text-gray-800">单人反馈</h3><p className="mt-1 text-sm text-gray-500">为一名学生按当前课次生成；未选课次时按最近天数汇总。</p></div>
        <div className="flex flex-wrap items-end gap-3"><label className="grid gap-1 text-xs font-medium text-gray-500">学生<select value={singleStudentId} onChange={(event) => setSingleStudentId(event.target.value)} className="rounded-lg border border-gray-300 px-3 py-2 text-sm"><option value="">选择学生</option>{students.filter((student) => !className || student.class === className).map((student) => <option key={student.id} value={student.id}>{student.name}（{student.class}）</option>)}</select></label>{!sessionCode && <label className="grid gap-1 text-xs font-medium text-gray-500">时间范围<select value={singleDays} onChange={(event) => setSingleDays(Number(event.target.value))} className="rounded-lg border border-gray-300 px-3 py-2 text-sm"><option value={7}>近 7 天</option><option value={14}>近 14 天</option><option value={30}>近 30 天</option></select></label>}<button onClick={generateSingleFeedback} disabled={!singleStudentId || singleLoading} className="rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{singleLoading ? "生成中…" : "生成单人反馈"}</button></div>
        {singleFeedback && <textarea value={singleFeedback} onChange={(event) => setSingleFeedback(event.target.value)} className="mt-4 min-h-32 w-full rounded-lg border border-gray-300 p-3 text-sm leading-6" />}
      </section>
    </div>
  );
}
